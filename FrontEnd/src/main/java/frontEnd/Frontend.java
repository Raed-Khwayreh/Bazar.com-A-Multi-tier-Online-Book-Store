...
static boolean to_cat=true;
	...
	public static void update(String id) {
		for(int i =0 ; i<cache.length ; i++) 
			if(cache[i].id.equals(id)&&!cache[i].quantity.equals("0")) cache[i].quantity=String.valueOf(Integer.parseInt(cache[i].quantity)-1);
	...
	public static void main(String[] args) {	
		...
		get("/info/:id", (req, res) -> {
			 ...
			 URL url = new URL("http://172.19.2.183:"+port+"/info/"+req.params(":id"));
		...
		post("/info", (request, response) -> {
			...
			if(bookCheck!=null) {
			 ...
			 URL url = new URL("http://172.19.2.183:"+port+"/info/"+bookID.id);
		...
		get("/search/:cateName", (request, response) -> {
			...
			response.type("application/json");
			 ...
			 URL url = new URL("http://172.19.2.183:"+port+"/search/"+request.params(":cateName").replace(" ","%20"));
		...
		post("/search", (request, response) -> {
			...
			Book bookCategory = new Gson().fromJson(request.body(), Book.class);
			 ...
			 System.out.println("From Catalouge");
				...
				to_cat=!to_cat;
			 URL url = new URL("http://172.19.2.183:"+port+"/search/"+bookCategory.category.replace(" ","%20"));
			...
			Book bookCheck = getBookByID(request.params(":id"));
			 if(bookCheck!=null) {
				 System.out.println("From Cache");
					 ...
					 URL url = new URL("http://172.19.2.183:"+port+"/purchase/"+request.params(":id"));
					 ...
					 bookCheck = getBookByID(request.params(":id"));
					 BufferedReader in = new BufferedReader(new InputStreamReader(con.getInputStream()));
					 return  new Gson().toJson(bookCheck);
				 }
				...
				Book bookCheck = getBookByID(request.params(":id"));
				 if(bookCheck!=null) {
					 System.out.println("From Cache");
						 ...
						 URL url = new URL("http://172.19.2.183:"+port+"/purchase/"+bookID.id);
					 ...
					 BufferedReader in = new BufferedReader(new InputStreamReader(con.getInputStream()));
							...
							System.out.println(content);
								...
								if(!content.equals("Book not exist")) {
									Book book_cash = new Book(s[0],s[1],s[2],s[3],s[4]);
									Book book_show = new Book(s[0],s[1],s[2]);
